...
//                    if(&& !$this->skip_new && 'import' ==  $this->found_action){
//                    }
                }
...
//            $password = (strlen($data['user_details']['user_pass']) == 34 ) ? $data['user_details']['user_pass'] : wp_hash_password($data['user_details']['user_pass']);
        ...
        if (is_email($customer_email) || $create_user_without_email === TRUE) {
            ...
            if (!is_wp_error($found_customer)) {
                ...
                $new_roles_str = str_replace(' ', '', isset($data['user_details']['roles']) ? $data['user_details']['roles'] : array() );
...
//                    $meta = trim(str_replace('meta:', '', $meta));
					...
					//$final_user_meta_string.="('$found_customer', '$key', '$meta_value'),";
                }
				
				//$final_user_meta_string = rtrim($final_user_meta_string, ",");
				//if($final_user_meta_string != '')
				//$wpdb->query("INSERT INTO $wpdb->usermeta (user_id, meta_key, meta_value) VALUES $final_user_meta_string");
...
Thanks for creating an account on %s. Your username is %s. You can access your account area to view orders, change your password.

We look forward to seeing you soon.

%s"),$first_name,$blog_name,$username,$blog_name);
...
Your password has been generated: %s

We look forward to seeing you soon.

%s"),$first_name,$blog_name,$username,$new_password,$blog_name);
...
%s"),$first_name,$blog_name,$username,$password,$blog_name);
                    ...
                    if(isset($mail_template['enable_email_templates']) && $mail_template['enable_email_templates'] == 1){
