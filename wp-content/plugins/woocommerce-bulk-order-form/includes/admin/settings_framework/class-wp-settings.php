...
class WooCommerce_Bulk_Order_Form_WP_Settings {
     ...
     * @return bool|void
	 */
	public function init( array $pages, string $page_hook = '' ) {
		$this->pages     = $pages;
		$this->page_hook = trim( sanitize_title( $page_hook ) );
	...
	private function add_settings_sections(): void {
		if ( isset( $this->current_page['sections'] ) ) {
	 ...
	 */
	private function add_settings_fields( string $sections_id, array $fields, string $page_id ): void {
		$opt_defaults = array();
		$defaults     = array(
			'section' => $sections_id,
			'id'      => '',
		...
		$use_defaults = false === get_option( $sections_id );

		foreach ( $fields as $field ) {

			// Field (rows) can be added by external scripts.
			$multiple = isset( $field['fields'] ) && $field['fields'];
		...
		foreach ( $this->pages as $page ) {
			if ( isset( $page['sections'] ) ) {
				foreach ( $page['sections'] as $section ) {
					...
					if ( isset( $section['sanitize_callback'] ) && $section['sanitize_callback'] ) {
						register_setting( $page_id, $sections_id, $section['sanitize_callback'] );
					} else {
	 ...
	 *
	 */
	public function add_pages( array $pages ): array {
	 ...
	 *
	 */
	public function add_section( string $page, array $section ): array {
	 ...
	 *
	 */
	public function add_sections( string $page, array $sections ): array {
	 ...
	 */
	public function add_field( string $page, string $section, array $field ): array {
		foreach ( $this->pages as $key => $_page ) {
	 ...
	 */
	public function add_fields( string $page, string $section, array $fields ): array {
		foreach ( $fields as $field ) {
	 ...
	 * @return void
	 */
	public function render_section_description( array $section ): void {
	 ...
	 * @param bool|string $tab_id Page id. Manually set the active tab.
	 *
	 * @return void
	 */
	public function render_header( string $plugin_title = '', $tab_id = false ): void {
		if ( ! empty( $plugin_title ) ) {
			echo '<h2>' . esc_attr( $plugin_title ) . '</h2>';
		}
		...
		foreach ( $this->pages as $page ) {
			...
			if ( ( isset( $page['title'] ) && $page['title'] ) ) {
				if ( $page_title_count > 1 ) {
					...
					$html .= sprintf( '<a href="%1$s" class="nav-tab%2$s" id="%3$s-tab">%4$s</a>', esc_url( $tab_url ), esc_attr( $active ), esc_attr( $page['id'] ), esc_html( $page['title'] ) );

					$html .= ( ++$i === $page_title_count ) ? '</h2>' : '';
				}

				if ( $page_title_count === 1 ) {
					if ( isset( $current['title'] ) && $current['title'] === $page['title'] ) {
						if ( ! empty( $page['title'] ) )
							$html .= '<h3>' . esc_html( $page['title'] ) . '</h3>';
						break;
					}
				}
	 ...
	 * @return void
	 */
	public function render_footer( string $plugin_title = '', $tab_id = false ): void {
