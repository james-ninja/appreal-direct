...
class RemotePost extends Http
{
    ...
    public function post($url, $data, $scope, $args = [], $expecting_serial = false)
    {
        ...
        $args = wp_parse_args(
            ...
            [
                ...
                'user-agent' => $this->util->get_requests_user_agent(),
                'blocking'   => true,
                'sslverify'  => $sslverify,
            ]
        );
    ...
    public function handle_remote_post_responses($response, $url, $scope, $expecting_serial, $state_data = array())
    {
        ...
        if (
            ...
            if ($scope === 'verify_connection_to_remote_site') {
     ...
     *
     * @return mixed|WP_Error Response if valid, error otherwise
     */
    public function verify_remote_post_response($response)
    {
        if (false === $response) {
            ...
            $this->error_log->log_error($error_msg, $return);
        ...
        if (isset($response['wpmdb_error'])) {
            ...
            return new WP_Error('wpmdb_error', $response['wpmdb_error']);
        }
