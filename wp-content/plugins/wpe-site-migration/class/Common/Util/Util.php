...
class Util
{
    ...
    public function site_details($state_data = [])
    {
        ...
        if ( ! class_exists('WP_Debug_Data') && ! version_compare($wp_version, '5.2', '<')) {
            require_once(ABSPATH . 'wp-admin/includes/class-wp-debug-data.php');
        }

        if (class_exists('WP_Debug_Data')) {
            foreach (WP_Debug_Data::get_sizes() as $key => $value) {
                if (key_exists($key, $sizes) && isset($value['raw']) && is_int($value['raw'])) {
                    $sizes[$key] = $value['raw'];
                }
            }
        }

        $site_details['sizes'] = $sizes;
    ...
    {
        return ! (defined('DOING_CRON') && DOING_CRON) && ! (defined('WP_CLI') && WP_CLI) && ! self::is_doing_mdb_rest() && ! self::wpmdb_is_ajax() && ! is_admin();
    }
    ...
    public static function get_stage_base_dir($stage)
    {
        ...
        $dirs          = [
            ...
            'muplugin_files' => WPMU_PLUGIN_DIR,
    ...
    public static function strip_tags_from_array(array $to_strip)
    {
        ...
        foreach ($to_strip as $key => $item) {
            ...
            $stripped[$key] = strip_tags($item, '<a><br>');
        }
