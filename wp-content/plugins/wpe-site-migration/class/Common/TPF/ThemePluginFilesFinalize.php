...
class ThemePluginFilesFinalize
{
    ...
    public function verify_file_transfer($state_data)
    {
        ...
        if (isset($current_migration['stages']) && in_array('theme_files', $current_migration['stages']) && isset($state_data['theme_folders'])) {
            $stages[] = 'themes';
        }

        if (isset($current_migration['stages']) && in_array('plugin_files', $current_migration['stages']) && isset($state_data['plugin_folders'])) {
            $stages[] = 'plugins';
        }

        if (isset($current_migration['stages']) && in_array('muplugin_files', $current_migration['stages']) && isset($state_data['muplugin_folders'])) {
            $stages[] = 'muplugins';
        }

        if (isset($current_migration['stages']) && in_array('other_files', $current_migration['stages']) && isset($state_data['other_folders'])) {
            $stages[] = 'others';
        }
