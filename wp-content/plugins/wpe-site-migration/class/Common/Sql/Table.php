...
class Table
{
    ...
    public function get_sql_dump_info($migration_type, $info_type)
    {
        ...
        $dump_info = sprintf('%s%s%s-%s.sql', Filesystem::get_upload_info($info_type), $ds, $dump_name, $session_salt);
    ...
    function process_table($table, $fp = null, $state_data = [])
    {
        ...
        $subsite_migration = array_key_exists('mst_select_subsite',
                $state_data) && '1' === $state_data['mst_select_subsite'];
        ...
        do {
            if ('import' === $state_data['intent'] && 'find_replace' === $state_data['stage']) {
        ...
        foreach ($queries as $query) {
            if (false === $wpdb->query($query)) {
                ...
                if (is_wp_error($return)) {
                    return new WP_Error('wp-migrate-db-strip-invalid', $return->get_error_message());
                } elseif (true !== $return) {
            ...
            if ($this->wpdb->query_has_invalid_text($query)) {
                if ( ! (defined('WPMDB_STRIP_INVALID_TEXT') && WPMDB_STRIP_INVALID_TEXT)) {
                    ...
                    if (false === $wpdb->query($this->wpdb->last_stripped_query)) {
                        ...
                        $return = new WP_Error(
                            'strip_invalid_text_query_failed',
                            'Failed to import the stripped SQL query: ' . $error
                        );
                    }
                }
            } else {
                // Can't clean up and retry.
                $return = false;
            }
        }
        ...
        if (!$skip_row) {
            ...
            foreach ($row as $key => $value) {
                ...
                if (isset($structure_info['points'][strtolower($key)]) && $structure_info['points'][strtolower($key)]) {
                    ...
                    $should_create_geom = is_array($unpacked) && array_key_exists('lon', $unpacked) && array_key_exists('lat', $unpacked);
    ...
    public function transfer_chunk($fp, $state_data, $table)
    {
        ...
        if ($state_data['intent'] === 'pull') {
            return $this->row_tracker . ',' . serialize($this->primary_keys);
        }
