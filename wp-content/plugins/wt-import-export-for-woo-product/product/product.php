...
class Wt_Import_Export_For_Woo_Product {
    ...
    */
    public function importer_steps($steps, $base)
    {
        ...
        {
            $steps['advanced']['description']=__('Use advanced options from below to decide updates to existing products, batch import count or schedule an import. You can also save the template file for future imports.', 'wt-import-export-for-woo');
        }
        return $steps;
    }
...
//        $colunm = array_map(function($vl){ return array('title'=>$vl, 'description'=>$vl); }, $arr); 
    ...
    public static function get_all_metakeys() {
        ...
        $meta = $wpdb->get_col(
            ...
            WHERE p.post_type IN ( 'product', 'product_variation' ) LIMIT 2000"
        );
    ...
    public function exporter_alter_advanced_fields($fields, $base, $advanced_form_data) {
        ...
        $out['export_children_sku'] = array(
            'label' => __("Export children SKU of grouped products", 'wt-import-export-for-woo'),
            'type' => 'radio',
            'radio_fields' => array(
            ...
            'help_text' => __("Option 'Yes' exports children of grouped products by their SKU. Default is Product ID.", 'wt-import-export-for-woo'),
        );
        ...
        $out['image_export'] = array(
            'label' => __("Export images as zip file", 'wt-import-export-for-woo'),
            'type' => 'image_export',
            'value' => 'No',
            'field_name' => 'image_export',
            'field_group'=>'advanced_field',
            'help_text' => sprintf(__('This option downloads the product images in a separate zip file. The exported CSV will contain the name of the images instead of their URL path. Use this option if you have a large number of products to import or if you experience slowness during the import process. %sLearn More.%s', 'wt-import-export-for-woo'),'<a href="https://www.webtoffee.com/exporting-importing-woocommerce-products-images-with-zip-file/" target="_blank">','</a>'),
        ); 
    ...
    public function importer_alter_advanced_fields($fields, $base, $advanced_form_data) {
        ...
        $out['skip_new'] = array(
            'label' => __("Skip new products", 'wt-import-export-for-woo'),
            'type' => 'radio',
            'radio_fields' => array(
        ...
        $out['conflict_with_existing_post'] = array(
            ...
            'help_text' => __('All the items within WooCommerce/WordPress are treated as posts and assigned a unique ID as and when they are created in the store. The post ID uniquely identifies an item irrespective of the post type be it product/pages/attachments/revisions etc.', 'wt-import-export-for-woo'),
            'help_text_conditional'=>array(
                array(
                    'help_text'=> __('If the post ID of the product being imported exists already(for any of the other post types like coupon, order, pages, media etc) skip the product from being inserted into the store.', 'wt-import-export-for-woo'),
                    'condition'=>array(
                        array('field'=>'wt_iew_id_conflict', 'value'=>'skip')
                    )
                ),
                array(
                    'help_text'=> __('Insert the product into the store with a new product ID(next available post ID) different from the value in the input file.', 'wt-import-export-for-woo'),
                    'condition'=>array(
        ...
        $out['merge_with'] = array(
            ...
            'help_text_conditional'=>array(
                array(
                    'help_text'=> __('If the post ID of the product being imported exists already(for any of the other post types like coupon, order, user, pages, media etc) skip the product from being updated into the store.', 'wt-import-export-for-woo'),
                    'condition'=>array(
        ...
        $out['found_action_merge'] = array(
            'label' => __("If product exists in the store", 'wt-import-export-for-woo'),
            'type' => 'radio',
            'radio_fields' => array(
//                'import' => __('Import as new item'),
            ...
            'help_text_conditional'=>array(
                array(
                    'help_text'=> __('Retains the product in the store as is and skips the matching product from the input file.', 'wt-import-export-for-woo'),
                    'condition'=>array(
                        array('field'=>'wt_iew_found_action', 'value'=>'skip')
                    )
                ),
                array(
                    'help_text'=> __('Update product as per data from the input file', 'wt-import-export-for-woo'),
                    'condition'=>array(
        ...
        $out['use_sku_upsell_crosssell'] = array(
            ...
            'help_text' => __('Enable to import up-sells,cross-sells and grouped products using the product SKU.', 'wt-import-export-for-woo'),
        );
...
//        );
        ...
        $out['delete_existing'] = array(
            ...
            'help_text' => __('Select ‘Yes’ if you need to remove products from your store which are not present in the input file. For e.g, if you have a product A in your store and your import file has products B, C; the product A is deleted from the store prior to importing B and C.', 'wt-import-export-for-woo'),
        );
    ...
    public function exporter_alter_filter_fields($fields, $base, $filter_form_data) {
		...
		$fields['limit']['help_text']=__('Exports specified number of products. e.g. Entering 500 with a skip count of 10 will export products from 11th to 510th position.', 'wt-import-export-for-woo');
        $fields['offset']['label']=__('Skip first <i>n</i> products', 'wt-import-export-for-woo');
		$fields['offset']['help_text']=__('Skips specified number of products from the beginning. e.g. Enter 10 to skip first 10 products from export.', 'wt-import-export-for-woo');

        $fields['product_types'] = array(
            'label' => __('Product types', 'wt-import-export-for-woo'),
