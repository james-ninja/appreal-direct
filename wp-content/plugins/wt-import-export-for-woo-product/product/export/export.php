...
//            if(1){ //if(is_woocommerce_prior_to('2.7.0')){
                $args['return']   = 'ids';
//            }
            ...
            // 'none', 'ID', 'name', 'type', 'rand', 'date', 'modified'
...
//            );
            ...
            if (1) { //if (is_woocommerce_prior_to('2.7.0')) {
                foreach ($products_ids as $key => $product_id) {  
                    ...
                    if('export_image' == $step){                        
                        if ($product_id) {                            
                            $image_array[] = self::getProductImages($product_id);                            
                        }                        
                    }else{                        
                        ...
                        if (!empty($children_ids)) {
                            foreach ($children_ids as $id) {                                
                                if(!in_array($id, $products_ids)){  // skipping if alredy processed in $products_ids                              
                                    if('export_image' == $step){                        
                                        if ($id) {                            
                                            $image_array[] = self::getProductImages($id);                            
                                        }                                    
                                    }else{                                                                
    ...
    public static function getProductImages($product) {
        ...
        // Featured image
        if (( $featured_image_id = get_post_thumbnail_id($product))) {
    ...
    protected function generate_row_data_wc_lower($product_object) {
        ...
        foreach ($csv_columns as $column => $value) {
            ...
            if (!$export_columns || in_array($value, $export_columns) || in_array($column, $export_columns)) {
                ...
                if (!wt_iew_is_woocommerce_prior_to('2.7')) {
