...
use Barn2\Plugin\WC_Bulk_Variations\Dependencies\Barn2\Setup_Wizard\Util;
/**
...
class Welcome extends Step
{
    ...
    {
        $fields = [];
        $fields['license_key'] = ['type' => 'license', 'label' => esc_html__('License key', 'woocommerce-bulk-variations'), 'description' => esc_html__('Enter your license key to start using the plugin.', 'woocommerce-bulk-variations'), 'tooltip' => esc_html__('The licence key is contained in your order confirmation email.', 'woocommerce-bulk-variations')];
        return $fields;
    }
    /**
     * {@inheritdoc}
    ...
    public function get_license_details()
    {
        ...
        if (!current_user_can('manage_options')) {
            $this->send_error(__('You are not allowed to retrieve license details.', 'woocommerce-bulk-variations'));
        }
    ...
    public function submit()
    {
        check_ajax_referer('barn2_setup_wizard_nonce', 'nonce');
        if (!current_user_can('manage_options')) {
            $this->send_error(__('You are not allowed to validate your license.', 'woocommerce-bulk-variations'));
        }
        $license_key = isset($_POST['license_key']) && !empty($_POST['license_key']) ? sanitize_text_field($_POST['license_key']) : \false;
        if (!$license_key) {
            $this->send_error(esc_html__('Please enter a license key.', 'woocommerce-bulk-variations'));
        }
