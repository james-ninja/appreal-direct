<?php
namespace Barn2\WBV_Lib\Plugin\License\Admin;

use Barn2\WBV_Lib\Registerable,
    Barn2\WBV_Lib\Plugin\License\License,
    Barn2\WBV_Lib\Admin\Settings_API_Helper,
    WC_Admin_Settings;
...
class License_Key_Setting implements Registerable, License_Setting {
        ...
        if ( $this->is_license_action( self::ACTIVATE_KEY ) ) {
            ...
            if ( isset( $license_setting['license'] ) ) {
                ...
                $this->add_settings_message(
                    __( 'License key successfully activated.', 'woocommerce-bulk-variations' ),
                    __( 'There was an error activating your license key.', 'woocommerce-bulk-variations' ),
                    $activated
                );
            }
            ...
            $this->add_settings_message(
                __( 'License key successfully deactivated.', 'woocommerce-bulk-variations' ),
                __( 'There was an error deactivating your license key, please try again.', 'woocommerce-bulk-variations' ),
                $deactivated
            );
            ...
            $this->add_settings_message(
                __( 'The license key looks good!', 'woocommerce-bulk-variations' ),
                __( 'There\'s a problem with your license key.', 'woocommerce-bulk-variations' ),
    ...
    public function get_license_key_setting() {
        $setting = array(
            'title' => __( 'License key', 'woocommerce-bulk-variations' ),
            'type'  => 'text',
        ...
        if ( $this->is_woocommerce ) {
            $setting['desc_tip'] = __( 'The licence key is contained in your order confirmation email.', 'woocommerce-bulk-variations' );
        } elseif ( $this->is_edd ) {
            // EDD uses title case for setting names, so let's keep things consistent.
            $setting['title'] = __( 'License Key', 'woocommerce-bulk-variations' );
    ...
    private function get_license_description() {
        $buttons = array(
            ...
            'activate'   => $this->license_action_button( self::ACTIVATE_KEY, __( 'Activate', 'woocommerce-bulk-variations' ) ),
            'deactivate' => $this->license_action_button( self::DEACTIVATE_KEY, __( 'Deactivate', 'woocommerce-bulk-variations' ) )
        );
        ...
        if ( $this->license->is_active() ) {
            ...
            if ( $this->license->is_inactive() && $this->is_license_action( 'deactivate_key' ) ) {
                ...
                $message = __( 'License key deactivated.', 'woocommerce-bulk-variations' );
            } else {
