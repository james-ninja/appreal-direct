...
*/
add_action( 'ph_canada_post_shipment_tracking_ids', 'ast_pro_add_tracking_information_into_order_ph_canada_post', 10, 2  );	

if ( !function_exists( 'ast_pro_add_tracking_information_into_order_ph_canada_post' ) ) {
	function ast_pro_add_tracking_information_into_order_ph_canada_post( $shipment_id_cs, $order_id ) {
		if ( class_exists( 'AST_Pro_Actions' ) ) {
			if ( function_exists( 'ast_insert_tracking_number' ) ) {                
				...
				ast_insert_tracking_number( $order_id, wc_clean( $shipment_id_cs ), 'Canada Post', gmdate('Y-m-d'), $status_shipped );
			}
		}
	}
}
