...
class AST_Pro_Update_Manager {
	...
	*/
	...
	public function addUpgradeMessageLink() {		
		...
		if ( ast_pro()->license->get_license_status() ) {
			return;
		}	
		
		$url = admin_url( 'admin.php?page=woocommerce-advanced-shipment-tracking&tab=license' );
		
		/* translators: %1$s: replace with license link, %2$s: replace with </a> */
		echo sprintf( ' ' . esc_html__( 'To receive automatic updates license activation is required. Please visit %1$ssettings%2$s to activate your Advanced Shipment Tracking Pro.', 'ast-pro' ), '<a href="' . esc_url( $url ) . '" target="_blank">', '</a>' );
