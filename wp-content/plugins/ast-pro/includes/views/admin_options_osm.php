		...
		<tbody>					
			<tr valign="top">
				...
				<td style="text-align:right;">
					<a class='settings_edit' href="<?php echo esc_url( $ast_pro_customizer_settings->get_customizer_url( 'ast_tracking_general_section', 'settings' ) ); ?>"><?php esc_html_e( 'Customize', 'woocommerce' ); ?></a>
				</td>
			</tr>
			...
			foreach ( $osm_data as $o_status => $data ) {
				...
				<tr valign="top" class="<?php esc_html_e( $o_status ); ?>_row <?php esc_html_e( $disable_row ); ?>">	
					<td class="forminp">
						...
						<input class="ast-tgl ast-tgl-flat order_status_toggle" id="<?php esc_html_e( $data['id'] ); ?>" name="<?php esc_html_e( $data['id'] ); ?>" type="checkbox"  value="1" <?php esc_html_e( $checked ); ?> />
					...
					<td class="forminp status-label-column">
						<span class="order-label <?php esc_html_e( $data['label_class'] ); ?>">
							...
							if ( get_option( $data['id'] ) ) {
								esc_html_e( wc_get_order_status_name( $data['slug'] ), 'ast-pro' );	
							} else {
