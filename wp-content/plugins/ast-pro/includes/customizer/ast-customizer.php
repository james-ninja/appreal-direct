...
class Ast_Customizer {
	...
	public static $email_types_class_names  = array(		
		...
		'shipped'							=> 'WC_Email_Customer_Shipped_Order'		
	);
	...
	*/
	public function register_woocommerce_menu() {
		add_menu_page( __( 'AST Customizer', 'ast-pro' ), __( 'AST Customizer', 'ast-pro' ), 'manage_woocommerce', 'ast_customizer', array( $this, 'settingsPage' ) );
	}
	...
	*/
	public function settingsPage() {
		...
		$completed_label = ( $rename_shipped_status ) ? esc_html__( 'Shipped', 'ast-pro' ) : esc_html__( 'Completed', 'ast-pro' );		
		
		$custom_shipped = ( !$rename_shipped_status && $custom_shipped_status ) ? true : false;
		
		$email_types = array(
			'completed'		  => $completed_label,
			'partial_shipped' => esc_html__( 'Partially Shipped', 'ast-pro' ),		
		);

		if ( $custom_shipped ) {
			$email_types['shipped'] = esc_html__( 'Shipped', 'ast-pro' );
		}
				...
				<section class="zoremmail-layout zoremmail-layout-has-content zoremmail-layout-sider">
					...
					<aside class="zoremmail-layout-slider-header">
						...
						<span class="wcts-save-content" style="float: right;">
							<button name="save" class="button-primary button-trackship btn_large woocommerce-save-button" type="submit" value="Saved" disabled><?php esc_html_e( 'Saved', 'ast-pro' ); ?></button>
	...
	*/	
	public function customizer_enqueue_scripts() {
		...
		wp_enqueue_style( 'ast-customizer', plugin_dir_url(__FILE__) . 'assets/Customizer.css', array(), ast_pro()->version );
		wp_enqueue_script( 'ast-customizer', plugin_dir_url(__FILE__) . 'assets/Customizer.js', array( 'jquery', 'wp-util', 'wp-color-picker','jquery-tiptip' ), ast_pro()->version, true );
		
		wp_localize_script('ast-customizer', 'ast_customizer', array(
			'plugin_dir_url'		=> ast_pro()->plugin_dir_url(),
	...
	*/	
	public function ast_generate_defaults() {
		...
		$customizer_defaults = array(
			...
			'completed_additional_content' => __( 'Thanks for shopping with us.', 'woocommerce' ),								
			'partial_shipped_subject' => __( 'Your {site_title} order is now partially shipped', 'ast-pro' ),
			'partial_shipped_heading' => __( 'Your Order is Partially Shipped', 'ast-pro' ),
			'partial_shipped_email_content' => __( "Hi there. we thought you'd like to know that your recent order from {site_title} has been partially shipped.", 'ast-pro' ),	
			'partial_shipped_additional_content' => __( 'Thanks for shopping with us.', 'woocommerce' ),
			'shipped_subject' => __( 'Your {site_title} order is now Shipped', 'ast-pro' ),
			'shipped_heading' => __( 'Your Order is Shipped', 'ast-pro' ),
			'shipped_email_content' => __( "Hi there. we thought you'd like to know that your recent order from {site_title} has been shipped.", 'ast-pro' ),	
			'shipped_additional_content' => __( 'Thanks for shopping with us.', 'woocommerce' ),
			'display_shipping_address' => 1,
			'display_billing_address'  => 1,	
			'display_shippment_item_price' => 0,
			'display_product_images' => 1,
			'shipping_items_heading' => esc_html__( 'Items in this shipment', 'ast-pro' ),
		);
	...
	public function customize_setting_options_func() {
		...
		if ( $wc_ast_status_partial_shipped ) {
			$email_types['partial_shipped'] = esc_html__( 'Partially Shipped', 'ast-pro' );
		}
		...
		$settings = array(
			...
			'email_design'	=> array(
				'id'	=> 'email_design',
				'class' => 'options_panel',
				'label' => esc_html__( 'Tracking Widget', 'ast-pro' ),
			...
			'email_content'	=> array(
				'id'	=> 'email_content',
				'class' => 'options_panel',
				'label'	=> esc_html__( 'Email Content', 'ast-pro' ),
			...
			'widget_header' => array(
				'id'	=> 'widget_header',
				'title'	=> esc_html__( 'Widget Header', 'ast-pro' ),
				'parent_label' => esc_html__( 'Tracking Widget', 'ast-pro' ),
				'type'	=> 'sub-panel',
				'parent'=> 'email_design',
				'show'	=> true,
				'class' => 'sub_options_panel',
			...
			'tracking_button' => array(
				'id'	=> 'tracking_button',
				'title'	=> esc_html__( 'Tracking Button', 'ast-pro' ),
				'parent_label' => esc_html__( 'Tracking Widget', 'ast-pro' ),
				'type'	=> 'sub-panel',
				'parent'=> 'email_design',
				'show'	=> true,
			...
			'email_settings' => array(
				'id'	=> 'email_settings',
				'title'	=> esc_html__( 'Email Content', 'ast-pro' ),
				'parent_label' => esc_html__( 'Email Content', 'ast-pro' ),
				'type'	=> 'sub-panel',
				'parent'=> 'email_content',
				'show'	=> true,
				'class' => 'sub_options_panel',
			...
			'hide_trackig_header' => array(
				'title'    => esc_html__( 'Hide tracking header', 'ast-pro' ),
			...
			'header_text_change' => array(
				'title'    => esc_html__( 'Tracking header text', 'ast-pro' ),
			...
			'additional_header_text' => array(
				'title'    => esc_html__( 'Additional text after header', 'ast-pro' ),
			...
			'fluid_display_shipped_header' => array(
				'title'    => esc_html__( 'Display shipment status section', 'ast-pro' ),
			...
			'fluid_tracker_type' => array(
				'title'    => esc_html__( 'Tracker type', 'ast-pro' ),
				'type'     => 'select',
			...
			'fluid_table_background_color' => array(
				'title'    => esc_html__( 'Background color', 'ast-pro' ),
				'type'     => 'color',
			...
			'fluid_table_border_color' => array(
				'title'    => esc_html__( 'Border color', 'ast-pro' ),
				'type'     => 'color',
			...
			'fluid_table_border_radius' => array(
				'title'    => esc_html__( 'Border radius', 'ast-pro' ),
				'type'     => 'range',
				'min'	   => 0,	
				'max'	   => 20,
			...
			'fluid_hide_provider_image' => array(
				'title'    => esc_html__( 'Hide shipping provider image', 'ast-pro' ),
			...
			'fluid_button_text' => array(
				'title'    => esc_html__( 'Button text', 'ast-pro' ),
			...
			'fluid_button_size' => array(
				'title'    => esc_html__( 'Button size', 'ast-pro' ),
				'type'     => 'radio_butoon',
			...
			'fluid_button_background_color' => array(
				'title'    => esc_html__( 'Button color', 'ast-pro' ),
				'type'     => 'color',
			...
			'fluid_button_font_color' => array(
				'title'    => esc_html__( 'Button font color', 'ast-pro' ),
				'type'     => 'color',
			...
			'fluid_button_radius' => array(
				'title'    => esc_html__( 'Button radius', 'ast-pro' ),
				'type'     => 'range',
				'min'	   => 1,	
				'max'	   => 20,
		...
		//sections			
		$all_statuses = array(
			'completed'		=> esc_html__( 'Completed', 'ast-pro' ),
			'partial_shipped'			=> esc_html__( 'Partially Shipped', 'ast-pro' ),
			'shipped'	=> esc_html__( 'Shipped', 'ast-pro' ),
		);
		...
		$settings[ 'order_status' ] = array(
			'title'    => esc_html__( 'Email type', 'ast-pro' ),
			'type'     => 'select',
			'default'  => $email_type,
			'show'     => true,
			'options'  => $email_types,
		...
		foreach ( $all_statuses as $key => $value ) {
			...
			$settings[ $key . '_subject' ] = array(
				'title'    => esc_html__( 'Email Subject', 'ast-pro' ),
			...
			$settings[ $key . '_heading' ] = array(
				'title'    => esc_html__( 'Email heading', 'ast-pro' ),
			...
			$settings[ $key . '_additional_content' ] = array(
				...
				'default'  => !empty( $email_settings['additional_content']) ? stripslashes( $email_settings['additional_content'] ) : $this->defaults[$key . '_additional_content'],
				'placeholder' => $this->defaults[$key . '_additional_content'],
				'type'     => 'textarea',
				'show'     => true,
				'class'	=> 'heading',
				'option_name' => 'woocommerce_customer_' . $key . '_order_settings',
			...
			$settings[ $key . '_codeinfoblock' ] = array(
				'title'    => esc_html__( 'Available Placeholders:', 'ast-pro' ),
				'default'  => '<code>{customer_first_name}<br>{customer_last_name}<br>{site_title}<br>{order_number}<br>{order_date}<br>{tracking_number}<br>{shipping_provider}<br>{shipping_date}</code>',
				'type'     => 'codeinfo',
				'show'     => true,
				'class'		=> $key . '_sub_menu all_status_submenu',
			);		
			$settings[ $key . '_shipping_items_heading' ] = array(
				'title'    => esc_html__( 'Pickup items Heading Text', 'ast-pro' ),
				'default'  => !empty($email_settings['shipping_items_heading']) ? $email_settings['shipping_items_heading'] : $this->defaults['shipping_items_heading'],
				'placeholder' => esc_html__( 'Items in this shipment', 'ast-pro' ),
				'type'     => 'text',
				'show'     => true,
				'class'	   => 'shipping_items_heading',
				'option_name'=> 'woocommerce_customer_' . $key . '_order_settings',
				'option_key'=> 'shipping_items_heading',
				'option_type'=> 'array',
				'class'		=> $key . '_sub_menu all_status_submenu shipping_items_heading',
			);
			$settings[ $key . '_display_shippment_item_price' ] = array(
				'title'    => esc_html__( 'Display shipment item price', 'ast-pro' ),
				'default'  => isset($email_settings['display_shippment_item_price']) ? $email_settings['display_shippment_item_price'] : $this->defaults['display_shippment_item_price'],
				'type'     => 'checkbox',
				'show'     => true,
				'option_name'=> 'woocommerce_customer_' . $key . '_order_settings',
				'option_key'=> 'display_shippment_item_price',
				'option_type'=> 'array',
				'class'		=> $key . '_sub_menu all_status_submenu display_shippment_item_price',
			);
			$settings[ $key . '_display_product_images' ] = array(
				'title'    => esc_html__( 'Display product images', 'ast-pro' ),
				'default'  => isset( $email_settings['display_product_images'] ) ? $email_settings['display_product_images'] : $this->defaults['display_product_images'],
				'type'     => 'checkbox',
				'show'     => true,
				'option_name'=> 'woocommerce_customer_' . $key . '_order_settings',
				'option_key'=> 'display_product_images',
				'option_type'=> 'array',
				'class'		=> $key . '_sub_menu all_status_submenu display_product_images',
			);
			$settings[ $key . '_display_shipping_address' ] = array(
				'title'    => esc_html__( 'Display Shipping Address', 'ast-pro' ),
				'default'  => isset( $email_settings['display_shipping_address'] ) ? $email_settings['display_shipping_address'] : $this->defaults['display_shipping_address'],
				'type'     => 'checkbox',
				'show'     => true,
				'option_name'=> 'woocommerce_customer_' . $key . '_order_settings',
				'option_key'=> 'display_shipping_address',
				'option_type'=> 'array',
				'class'		=> $key . '_sub_menu all_status_submenu display_shipping_address',
			);
			$settings[ $key . '_display_billing_address' ] = array(
				'title'    => esc_html__( 'Display Billing Address', 'ast-pro' ),
				'default'  => isset( $email_settings['display_billing_address'] ) ? $email_settings['display_billing_address'] : $this->defaults['display_billing_address'],
				'type'     => 'checkbox',
				'show'     => true,
				'option_name'=> 'woocommerce_customer_' . $key . '_order_settings',
				'option_key'=> 'display_billing_address',
				'option_type'=> 'array',
				'class'		=> $key . '_sub_menu all_status_submenu display_billing_address',
			);		
		}
		...
		<div class="customize-section-title">
			<h3>
				<span class="customize-action-default">
					<?php esc_html_e( 'You are customizing', 'ast-pro' ); ?>
				</span>
				<?php esc_html_e( 'Email Customizer', 'ast-pro' ); ?>
			</h3>
		</div>
		<?php
							...
							<?php } else if ( isset($array['type']) && 'tgl-btn' == $array['type'] ) { ?>
								<div class="menu-sub-field">
									...
									<label class="menu-sub-title">
										...
										<label for="<?php esc_attr_e( $id ); ?>" class="shipment_email_label"><?php esc_html_e( 'Enable email', 'ast-pro' ); ?></label>
									</label>
								</div>
