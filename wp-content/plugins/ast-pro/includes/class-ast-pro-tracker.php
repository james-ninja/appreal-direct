...
class AST_PRO_Tracker {		
	...
	*/
	public function init() {
		...
		add_action( 'wp_ajax_ast_pro_activate_usage_data', array( $this, 'ast_activate_usage_data_fun') );
		add_action( 'wp_ajax_ast_pro_skip_usage_data', array( $this, 'ast_skip_usage_data_fun') );	
	...
	public function ast_activate_usage_data_fun() {
		...
		if ( isset( $_POST[ 'ast_pro_optin_email_notification' ] ) && 0 == $_POST[ 'ast_pro_optin_email_notification' ] && isset( $_POST[ 'ast_pro_enable_usage_data' ] ) && 0 == $_POST[ 'ast_pro_enable_usage_data' ] ) {
			update_option( 'ast_pro_usage_data_selector', true );
			die();
		}

		if ( isset( $_POST[ 'ast_pro_optin_email_notification' ] ) ) {						
			update_option( 'ast_pro_optin_email_notification', wc_clean( $_POST[ 'ast_pro_optin_email_notification' ] ) );
		}

		if ( isset( $_POST[ 'ast_pro_enable_usage_data' ] ) ) {						
			update_option( 'ast_pro_enable_usage_data', wc_clean( $_POST[ 'ast_pro_enable_usage_data' ] ) );			
		}
	...
	public function ast_skip_usage_data_fun() {
		...
		update_option( 'ast_pro_usage_data_selector', true );
		update_option( 'ast_pro_optin_email_notification', 0 );
		update_option( 'ast_pro_enable_usage_data', 0 );

		$this->set_unset_usage_data_cron();
	}

	public function set_unset_usage_data_cron() {
		$ast_pro_enable_usage_data = get_option( 'ast_pro_enable_usage_data', 0 );
		$ast_pro_optin_email_notification = get_option( 'ast_pro_optin_email_notification', 0 );

		if ( 0 == $ast_pro_enable_usage_data && 0 == $ast_pro_optin_email_notification ) {
	...
	public function get_tracking_data() {
		...
		$data['opt_in'] = $ast_pro_optin_email_notification;
		$data['enable_usage_data'] = $ast_pro_enable_usage_data;
		$data['plugin'] = 'Advanced Shipment Tracking Pro';
		
		if ( 1 == $ast_pro_enable_usage_data ) {
	...
	public function get_theme_info() {
		...
		return array(
			'name'        => $theme_name, // @phpcs:ignore
			'version'     => $theme_version, // @phpcs:ignore
