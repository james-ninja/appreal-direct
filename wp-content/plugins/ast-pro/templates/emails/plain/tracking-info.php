...
if ( $tracking_items ) {
	...
	echo esc_html( strtoupper( apply_filters( 'woocommerce_shipment_tracking_my_orders_title', __( $shipment_tracking_header, 'ast-pro' ) ) ) ) . "\n\n";
	...
	if ( isset( $new_status ) ) {
		...
		echo sprintf( esc_html__( 'Shipment status changed to %s', 'ast-pro' ), esc_html( apply_filters( 'trackship_status_filter', $new_status ) ) ) . "\n\n";
	}

	foreach ( $tracking_items as $tracking_item ) {
		echo esc_html__( $provider_header_text, 'ast-pro' ) . ': ' . esc_html( apply_filters( 'ast_provider_title', esc_html( $tracking_item['formatted_tracking_provider'] ) ) ) . "\n";
		echo esc_html__( $tracking_number_header_text, 'ast-pro' ) . ': ' . esc_html( $tracking_item['tracking_number'] ) . "\n";
		echo esc_html__( $track_header_text, 'ast-pro' ) . ': ' . esc_url( $tracking_item['ast_tracking_link'] ) . "\n\n";
	}
