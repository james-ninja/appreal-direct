    ...
    if (regex.test(jQuery("#trcking_csv_file").val().toLowerCase())) {
		if (typeof (FileReader) != "undefined") {
            ...
            reader.onload = function (e) {
				...
				jQuery(document).trigger( 'woocommerce_ast_pro_pre_import', trackings );
...
jQuery(document).on("click", ".edit_provider", function(){		
	...
	jQuery.ajax({
		...
		success: function(response) {
			...
			jQuery('input[name=make_provider_default]').attr('checked', false);			
			...
			if(provider == 'custom_provider'){				
				...
				if(response.default_provider == 1) {
					jQuery('input[name=make_provider_default]').attr('checked', true);
				}
...
jQuery(document).on("submit", "#paypal_tracking_settings_form", function(){
	'use strict';
...
jQuery(document).on("click", "#integrations_settings_form input[type=checkbox]", function( ){	
	var form = jQuery('#integrations_settings_form');
...
jQuery(document).on("click", ".accordion", function(){
	if ( jQuery(this).hasClass( 'active' ) ) {
		...
		jQuery(this).siblings( '.panel' ).removeClass( 'active' );
		...
		jQuery(this).siblings( '.panel' ).addClass( 'active' );
...
jQuery("#wc_ast_show_orders_actions,#wc_ast_unclude_tracking_info,#ast_order_display_in_fulfillment_dashboard,#ast_shipping_method_display_in_fulfillment_dashboard,#ptaa_payment_methods").on("change", function (e) { 
	jQuery('span.ast-accordion-btn button').prop("disabled", false);  
});

jQuery("#ptaa_client_id,#ptaa_client_secret").on("blur", function (e) { 
	jQuery('span.ast-accordion-btn button').prop("disabled", false);  
});
