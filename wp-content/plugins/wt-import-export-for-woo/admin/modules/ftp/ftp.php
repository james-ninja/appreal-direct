...
class Wt_Import_Export_For_Woo_Ftp
{
	...
	*/
	public function remote_adapter($adapters, $action, $adapter)
	{
		if($adapter != "")
		{
			if('ftp' == $adapter)
			{
				$adapters['ftp']=include plugin_dir_path(__FILE__).'classes/class-ftpadapter.php';
			}
		}else
		{
			$adapters['ftp']=include plugin_dir_path(__FILE__).'classes/class-ftpadapter.php';
	...
	*/
	public function test_ftp($out)
	{
    	...
    	{
    		$val=Wt_Iew_Sh::sanitize_data((isset($_POST[ $ftp_form_field ]) ? $_POST[ $ftp_form_field ] : ''), $ftp_form_field, $this->ftp_form_validation_rule);
