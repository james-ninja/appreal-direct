	...
	public function get_meta_mapping_fields($out)
	{
		...
		if($meta_mapping_screen_fields && is_array($meta_mapping_screen_fields))
		{
			...
			foreach($meta_mapping_screen_fields as $meta_mapping_screen_field_key=>$meta_mapping_screen_field_val)
			{
				...
				if ( sizeof( $mapping_fields ) > 2000 ) {
					$heavy_meta[ 'status' ] = TRUE;
					$heavy_meta[ 'message' ]			 = __( 'Large number of meta data has been detected. If you choose to proceed, this action will import only 2000 meta columns ( current website meta + new meta from the input file, in sequence). Reconsider removing unnecessary columns from input file and try again.' );
					$heavy_meta[ 'count' ]			 = sizeof( $mapping_fields );
