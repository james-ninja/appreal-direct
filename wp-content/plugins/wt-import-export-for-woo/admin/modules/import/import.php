...
{	
        ...
        public function __construct()
	{
		...
		$this->steps=array(
			'post_type'=>array(
				...
				'description'=>__('Import the respective post type from a CSV/XML/TXT. As a first step you need to choose the post type to start the import.'),
			),
			'method_import'=>array(
			...
			'mapping'=>array(
				...
				'description'=>__('Map the standard/meta/attributes/taxonomies and hidden meta columns with your CSV/XML column names.'),
			),
			'advanced'=>array(
				'title'=>__('Advanced options/Batch import/Scheduling'),
	...
	*/
	public function advanced_setting_fields($fields)
	{
		$fields['enable_import_log']=array(
			...
			'help_text'=>__('Save import log as text file and make it available in the log section for debugging purposes.'),
	...
	*/
	public function get_method_import_screen_fields($method_import_form_data)
	{
		...
		{
			$file_from_field_arr['label']=__("Enable FTP import?");
		...
		$method_import_screen_fields['date_format']=array(
			...
			'help_text'=>sprintf(__('Date format in the input file. Click %shere%s for more info about the date formats.'), '<a href="https://www.php.net/manual/en/function.date.php" target="_blank">', '</a>'),
                        'validation_rule'=>array('type'=>'skip'),
	...
	protected function enqueue_assets()
	{
		...
		$params=array(
			...
			'msgs'=>array(
				...
				'select_post_type'=>__('Please select a post type.'),
				...
				'drop_upload'=>__('Drag and Drop or click to upload'),
...
//				return $str;
        ...
        public function get_skip_from_evaluation(){
            return apply_filters('wt_iew_importer_skip_from_evaluation', array('description','post_content','short_description','post_excerpt','line_item_','shipping_items'));             
        }
        /*
         * 
         */
...
//                return wt_wc_format_decimal($value);
