var wt_iew_import=(function( $ ) {
	...
	var wt_iew_import=
	{
			...
			if(step=='post_type') /* post type page */
			{
				$('[name="wt_iew_import_post_type"]').unbind('change').change(function(){					
					...
					}else
					{
						...
						wt_iew_import.to_import_title=post_type_name;
		...
		{
		    var file_from = this.get_file_from();
		    var temporary_import_file = '';
		    if ( file_from == 'local' )
		    {
			temporary_import_file = $( '[name="wt_iew_local_file"]' ).val();
		    }
			    this.ajax_data = {
	            ...
	            'temp_import_file': (this.temp_import_file) ? this.temp_import_file : temporary_import_file,
	        };
