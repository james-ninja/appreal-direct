...
<div class="wt_iew_schedule_now wt_iew_popup">
	<div class="wt_iew_popup_hd">
		...
		<span class="wt_iew_popup_hd_label"><?php _e('Update Schedule');?></span>
...
<!--	<ul class="wt_iew_sub_tab">
	...
	<div class="wt_iew_schedule_now_box">
		<div class="wt_iew_cron_current_time"><b><?php _e('Current server time:');?></b> <span>--:--:-- --</span></div>
		...
		<div class="wt_iew_schedule_now_formrow">
			...
			<div class="wt_iew_schedule_type_box_single" style="margin-bottom:0px;">
                            <label for="wt_iew_schedule_wordpress_cron"><input type="radio" name="wt_iew_schedule_type" id="wt_iew_schedule_wordpress_cron" value="wordpress_cron" <?php checked('wordpress_cron', $cron_data['schedule_type'], true); ?> > <?php _e('Wordpress Cron');?> </label>
				<p class="wt_iew_schedule_type_desc"><?php _e('This type of scheduler depends on the Wordpress for scheduling your job at the specified time. However this model is dependent on your website visitors. Upon a visit Wordpress cron will check to see if the time/date is later than the scheduled event/s, and if it is– it will fire those events.');?></p>
			</div>
			<div class="wt_iew_schedule_type_box_single">
				<label for="wt_iew_schedule_server_cron"><input type="radio" name="wt_iew_schedule_type" id="wt_iew_schedule_server_cron" value="server_cron" <?php checked('server_cron', $cron_data['schedule_type'], true); ?>> <?php _e('Server Cron');?> </label>
				<p class="wt_iew_schedule_type_desc">
		...
		<div class="wt_iew_schedule_now_formrow">
			<input type="text" name="wt_iew_cron_file_name" value="<?php echo $cron_data['file_name'] ?>" /> <span class="wt_iew_cron_file_ext">.csv</span>
			<br />	
		...
		<div class="wt_iew_schedule_now_formrow">			
			<div class="wt_iew_schedule_now_interval_radio_block">
				<label for="wt_iew_cron_interval_day"><input type="radio" id="wt_iew_cron_interval_day" name="wt_iew_cron_interval" value="day" <?php checked('day', $cron_data['interval'], true); ?>> <?php _e('Every day');?></label>
				<label for="wt_iew_cron_interval_week"><input type="radio" id="wt_iew_cron_interval_week" name="wt_iew_cron_interval" value="week" <?php checked('week', $cron_data['interval'], true); ?>> <?php _e('Every week');?></label>
				<!-- <label for="wt_iew_cron_interval_biweek"><input type="radio" id="wt_iew_cron_interval_biweek" name="wt_iew_cron_interval" value="biweek"> <?php _e('Biweekly');?></label> -->
				<label for="wt_iew_cron_interval_month"><input type="radio" id="wt_iew_cron_interval_month" name="wt_iew_cron_interval" value="month" <?php checked('month', $cron_data['interval'], true); ?>> <?php _e('Every month');?></label>
				<label for="wt_iew_cron_interval_custom"><input type="radio" id="wt_iew_cron_interval_custom" name="wt_iew_cron_interval" value="custom" <?php checked('custom', $cron_data['interval'], true); ?>> <?php _e('Custom');?></label>
			</div>
			<div class="wt_iew_schedule_now_interval_sub_block wt_iew_schedule_custom_interval_block">
				<label><?php _e('Custom interval');?></label>
				<input type="number" step="1" min="1" name="wt_iew_cron_interval_val" value="<?php echo $cron_data['custom_interval']; ?>" placeholder="<?php _e('Interval in minutes.');?>">
			...
			<div class="wt_iew_schedule_now_interval_sub_block wt_iew_schedule_day_block">
				...
				<div class="wt_iew_schedule_now_interval_radio_block">				
					...
					foreach ($days as $day)
					{
						...
						?>
						<label for="wt_iew_cron_day_<?php echo $day_vl;?>"><input type="radio" value="<?php echo $day_vl;?>" id="wt_iew_cron_day_<?php echo $day_vl;?>" name="wt_iew_cron_day" <?php echo $checked;?>> <?php _e($day);?></label>
						<?php
					}
					?>
				</div>
			...
			<div class="wt_iew_schedule_now_interval_sub_block wt_iew_schedule_starttime_block">
				<label><?php _e('Start time');?></label> 
                                <div style="float:left">
                                    <input  type="number" step="1" min="1" max="12" name="wt_iew_cron_start_val" value="<?php echo $hour; ?>" />
                                    <span class="wt-iew_form_help" style="display:block; margin-top: 1px">Hour</span>
                                </div>
                                <div style="float:left">
                                    <span class="wt_iew_cron_start_val_min">:</span><input type="number" step="1" min="0" max="59" name="wt_iew_cron_start_val_min" value="<?php echo $minute; ?>" onchange="if(parseInt(this.value,10)<10)this.value='0'+this.value;" />
...
<!--			<div class="wt_iew_sub_tab_content" data-id="cron-file-details">
		...
		<div class="wt_iew_popup_footer">
			...
			<button type="button" name="" class="button-primary wt_iew_update_schedule" style="margin-right:20px;"><?php _e('Update Schedule');?></button>	
		</div>
