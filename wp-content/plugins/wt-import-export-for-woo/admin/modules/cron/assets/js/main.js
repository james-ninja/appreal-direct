var wt_iew_cron_js = ( function ( $ ) {
    ...
    var wt_iew_cron_js =
	{
	    ...
	    {
		jQuery( '.wt_iew_cron_url' ).click( function () {
		    window.prompt( wt_iew_cron_params.msgs.use_url, jQuery( this ).attr( 'data-href' ) );
		} );
	    },
	    reg_delete: function ()
	    {
		jQuery( '.wt_iew_delete_cron' ).click( function () {
	...
	{
		jQuery('.wt_iew_update_schedule').unbind('click').click(function(){
		...
		jQuery.ajax( {
		    ...
		    jQuery( '[name="wt_iew_cron_interval"]' ).unbind( 'click' ).click( function () {
			var vl = jQuery( this ).val();
			wt_iew_cron.toggle_interval_fields( vl );
		    } );

		    jQuery( '[name="wt_iew_schedule_type"]' ).unbind( 'click' ).click( function () {
			wt_iew_cron.hide_url_cron_fields();
		    } );
			...
			jQuery('[wf_frm_tgl-id="wt_iew_file_into"]').show();
			jQuery('[wf_frm_tgl-id="wt_iew_file_into"]').find('th label').animate({'margin-left':'25px'},1000);
		    }
		...
		{
			cron_settings_el.find('.wt_iew_sub_tab li').click(function(){
			...
			cron_settings_el.find('.wt_iew_sub_tab').each(function(){
