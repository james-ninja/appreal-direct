...
class Wt_Import_Export_For_Woo_Cron
{
	...
	public $action_modules=array('export'=>'export', 'import'=>'import');
	...
	*/
	public function ajax_main()
	{
		...
		$schedule_action=(isset($_REQUEST['iew_schedule_action']) ? sanitize_text_field($_REQUEST['iew_schedule_action']) : '');
	...
	*/
	public function admin_settings_page($args)
	{
		if(isset($_GET['wt_iew_change_schedule_status']) || isset($_GET['wt_iew_delete_schedule'])) 
		{
			if(Wt_Iew_Sh::check_write_access(WT_IEW_PLUGIN_ID))
			{
				$cron_id=absint($_GET['wt_iew_cron_id']);
				if($cron_id>0)
				{
					$cron_data=self::get_cron_by_id($cron_id);
					if($cron_data)
					{
						if(isset($_GET['wt_iew_delete_schedule'])) /* delete schedule action */
						{
						...
						{
							$action=sanitize_text_field($_GET['wt_iew_change_schedule_status']);
							if($action=='enable')
							{	
	...
	*/
	...
	{		
		foreach ($this->action_modules as $key => $value) 
		{
			...
			{
				if(wp_next_scheduled('wt_iew_do_cron_'.$key)) //its already scheduled then remove
				{
	...
	*/
	...
	public function enqueue_assets()
	{
		if(isset($_GET['page']))
		{
			if($_GET['page']==Wt_Import_Export_For_Woo::get_module_id('export') || $_GET['page']==Wt_Import_Export_For_Woo::get_module_id('import') || $_GET['page']==$this->module_id)
			{
				...
				$params=array(
					'msgs'=>array(
						...
						'date_selected_info'=>__(sprintf('You have selected %d as the date but this date is not available in all months. In that case, last date of the month will be taken. Proceed?', 30)),
	...
	*/
	...
	{	
		...
		*/
		...
		{
			$qry=$wpdb->prepare(
				"SELECT * FROM $tb WHERE ( ( (status= %d OR  status= %d) AND start_time <= %d ) OR status IN(%d, %d, %d) ) AND action_type=%s AND id=%d", 
				array(
					self::$status_arr['not_started'],
					self::$status_arr['finished'],
					$tme,
		...
		{ 
			...
			{
				if(defined('WT_IEW_DEBUG') && WT_IEW_DEBUG) /* debug */
				{
					echo "<pre>";
					print_r($cron_listv);
					echo "</pre><br />"; 					
	...
	*/
	...
	{
		global $wpdb;
		$cron_data=(isset($_POST['schedule_data']) ? Wt_Iew_Sh::sanitize_item($_POST['schedule_data'], 'text_arr') : null );
		if(!$cron_data)
		{
			return $out;
		}
		...
		$insert_data_type=array('%s', '%s', '%s', '%s', '%d', '%s', '%d', '%d', '%d', '%d');
	...
	public function edit_schedule( $out ) {
		...
		global $wpdb;				
		$cron_id = (isset($_POST['cron_id']) ? Wt_Iew_Sh::sanitize_item($_POST['cron_id'], 'int') : null );
		if(!$cron_id)
		{
			return $out;
		}
