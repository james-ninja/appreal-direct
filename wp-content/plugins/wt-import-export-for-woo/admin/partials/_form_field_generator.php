			...
			<td <?php echo $colspan_attr;?> class="<?php echo $td_class2;?>">
				...
				if($field_html=='')
				{
					...
					}elseif($type=='dropzone') /* dropzone file uploader */
					{
						...
						?>
						<?php
							$temp_import_file=(isset($_POST['temp_import_file']) ? ($_POST['temp_import_file']) : '');
							if( !empty($temp_import_file) && !wp_http_validate_url( $temp_import_file)){
								$temp_import_file = WP_CONTENT_URL.'/webtoffee_import/'.$temp_import_file;
							}

							$temp_import_file_name = basename($temp_import_file);
							if( wp_http_validate_url( $temp_import_file ) && strpos( $temp_import_file, 'local_file_') !== false ){
								$temp_import_file_name = isset($temp_import_file_name)  ? explode('_', $temp_import_file_name, 4)[3] : '';
							}
							
						?>
						<input <?php echo $css_attr;?> type="hidden" name="<?php echo $form_data_key; ?>" value="<?php echo !empty($temp_import_file) ? esc_url($temp_import_file) : esc_url($vl); ?>" <?php echo $attr;?> <?php echo ($html_id!="" ? $html_id : 'id="'.$field_id.'"');?>/>
						
						<div id="<?php echo $dropzone_id;?>" class="wt_iew_dropzone" wt_iew_dropzone_target="#<?php echo $field_id; ?>">
							<div class="dz-message">
								<?php _e('Drag and Drop or click to upload');?>
								<br /><br /><div class="wt_iew_dz_file_success"></div> <br />
								<div class="wt_iew_dz_file_name"><?php echo !empty($temp_import_file_name) ? $temp_import_file_name : '';?></div> <br />
