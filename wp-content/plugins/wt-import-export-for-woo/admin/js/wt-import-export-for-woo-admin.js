(function( $ ) {
	...
	$(function() {
		...
		wf_tab_view.Set('#'+wt_iew_params.plugin_id); /* set plugin main div as parent object to avoid conflict with other WT plugins */
		/* tab view */
...
var wt_iew_settings_form=
{
	Set:function(prnt_selector)
	{ 
		var wf_prnt_obj=jQuery(prnt_selector);
		wf_prnt_obj.find('.wt_iew_settings_form').submit(function(e){
			e.preventDefault();
			...
			jQuery.ajax({
				...
				type:'POST',
				dataType:'json',
				data:data+'&action=wt_iew_save_settings',
				...
				error:function () 
				{
					spinner.css({'visibility':'hidden'});
					submit_btn.css({'opacity':'1','cursor':'pointer'}).prop('disabled',false);
					wt_iew_notify_msg.error(wt_iew_params.msgs.settings_error);
				}
			});
		});
	}
...
var wt_field_group=
{
	Set:function()
	{
		...
		jQuery('.wt_iew_field_group_hd').unbind('click').click(function(){
...
var wt_iew_popover=
{
	...
	Set:function()
	{
		jQuery('[data-wt_iew_popover="1"]').unbind('click').click(function(){
			...
			if(jQuery('.wt_iew_popover').length==0)
			{
				...
				+'<button name="wt_iew_popover_do_action" type="button" class="button button-primary">'+wt_iew_params.msgs.use_expression+'</button>'
				+'<button name="popover_close" type="button" class="button button-secondary popover_close">'+wt_iew_params.msgs.cancel+'</button>'
			...
			{
				wt_iew_import.mapping_field_editor();
				wt_iew_import.mapping_field_editor_output_preview();
			}
	...
	{	
		jQuery('[name="wt_iew_popover_do_action"]').unbind('click').click(function(){
			...
			if(wt_iew_popover.action_module=='import')
			{
				var html_vl=wt_iew_import.mapping_field_editor_validate_column_val(vl);
			}
			...
			if(wt_iew_popover.action_module=='import')
			{
				...
				wt_iew_import.mapping_fields_selected_count(click_elm.parents('table'));
			}		
		});
	},
...
jQuery(document).ready(function(){
	wt_iew_popup.Set();
	wt_iew_settings_form.Set('#'+wt_iew_params.plugin_id);
	wt_drp_menu.Set();
