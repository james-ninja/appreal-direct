	...
	if ($user_login) {
		...
		if($user_obj->ID){
			...
			if($status == 'pending' || $status == 'disapproved'){
				...
				switch ( $status ) {
					case 'pending':
						$message = '<p> Your account is Pending from Admin Approval. currently you can not reset your password.</p>';
						break;
					case 'disapproved':
						$message = '<p> Your Account is Disapproved By Admin. currently you can not reset your password.</p>';
						break;
				}
				echo $message;
			}
